...
/* set bit 5 */
#define SET_OVERCURRENT(id_and_status) (id_and_status |= 0b00100000)
#define IS_OVERCCURRENT(id_and_status) ((id_and_status >> 5) & 1)
#define GET_STATUS(id_and_status)      (id_and_status & 1)
#define REROLL_COUNTER(id_and_status)                                                              \
	((id_and_status >> 1) & 1) // reroll counter to monitor the intern variables
...
void reception_function(void)
{
	...
	case (SLAVE_A):
		...
		if (rs_id == RS_ID_MASTER) {
			PUT_UPPER_12BITS(tx_consigne.buf_VcIac, I1_low_for_rs);
			...
			n_receive_calls = (n_receive_calls + 1);
		}
		break;
	case (SLAVE_B):
		if (rs_id == RS_ID_SLAVE_A) {
			PUT_LOWER_12BITS(tx_consigne.buf_IbW, I1_low_for_rs);
...
inline void get_measures()
{
	...
	I1_low_for_rs = to_12bits(I1_low_value, 40.0, 20.0);
...
{
	printk("%c: %s: %8.2f: %d\n", TXT_ID[SPIN_MODE], state_msg[control_state],
	       (double)((float32_t)total_ns * 0.001F), master_status);
...
__attribute__((section(".ramfunc"))) void loop_control_task()
{
	...
	if (control_state == POWER) {
		...
		if ( w0 > 25.0F) 
		{
			...
			duty_cycle_leg2 =
				1.0F / VOLTAGE_SCALE *
				(Vref + prop_res.calculateWithReturn(I1_low_value, I2_low_value));
		} 
		else
		{
			duty_cycle_leg2 = 1.0F / VOLTAGE_SCALE * Vref;
		}
